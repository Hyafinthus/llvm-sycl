...
struct DAGNode { // 一个kernel的依赖关系
  ...
  // Number of nodes in this node's linear partition-compatible component for
  // each supported Split width. A long resident chain amortizes its one-time
  // partition setup even when each individual stencil kernel is too short for
  // the ordinary cold-Split threshold.
  std::array<size_t, 5> persistent_chain_nodes{};
